...
- 継続作業を開始する際は、`handoffs/` 内の該当 `.md` ファイルを先に読んでから作業してください。
- 該当する引き継ぎ作業が完了したら、対応する `handoffs/` 内の `.md` ファイルを削除してください。
- `handoffs/README.md` と `.gitkeep` は削除対象ではありません。
- 上記の handoff ファイル削除については、作業完了時に確認を挟まず自律的に実行して構いません。
